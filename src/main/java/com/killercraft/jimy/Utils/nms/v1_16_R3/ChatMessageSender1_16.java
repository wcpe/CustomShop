package com.killercraft.jimy.Utils.nms.v1_16_R3;
...
import static net.minecraft.server.v1_16_R3.ChatClickable.EnumClickAction.RUN_COMMAND;
import static net.minecraft.server.v1_16_R3.ChatHoverable.EnumHoverAction.SHOW_TEXT;


public class ChatMessageSender1_16 {
    ...
    public static void sendMessage(Player player, IChatBaseComponent... siblings) {
        ...
        for (IChatBaseComponent component : siblings) text.getSiblings().add(component);
        handle.sendMessage(text,player.getUniqueId());
    }
    ...
    public static IChatBaseComponent format(String text, ChatClickable.EnumClickAction ca, String cv, ChatHoverable.EnumHoverAction ha, String hv) {
        ...
        ChatModifier style = ChatModifier.a;
        while (matcher.find()) {
            component.getSiblings().add(new ChatComponentText(text.substring(head, matcher.start()).replaceAll("&&", "&")).setChatModifier(style));
            style = parseStyle(matcher.group());
            head = matcher.end();
        }
        component.getSiblings().add(new ChatComponentText(text.substring(head).replaceAll("&&", "&")).setChatModifier(style));
        if (ca != null && cv != null) {
